...
async function main() {
	...
	try {
		...
		assert.equal((dom.window.document.getElementById('cornerRadiusRatio') as HTMLInputElement).value, '10', '圆角半径默认值应为 10x');
		...
		assert.equal(experimentalFastRestore.checked, true, '实验性性能加速默认必须开启');
		experimentalFastRestore.checked = false;
		experimentalFastRestore.dispatchEvent(new dom.window.Event('change', { bubbles: true }));
		await waitFor(() => storedSettings.experimentalFastRestore === false, '实验性性能加速设置应能关闭并保存');

...
- Selected 操作使用增量恢复，All 操作使用全量恢复；手动快照及其恢复前后安全快照也必须使用全量恢复。几何相同但操作范围或恢复策略不同的 Before 快照不能互相复用。
- Alpha 操作加速默认开启，但必须允许用户关闭、运行时检测能力并在 `finally` 恢复宿主计算状态；画布暂停代码只能预留且必须强制关闭，完成宿主实测后才能开放；美化只包裹图元变更与 DRC 修复阶段，批量覆铜等可选接口失败时回退既有路径，不能放宽全量清空与几何校验。

...
- Smooth Routing (Selected/All) – Process trace corners (arc-based beautification); the corner-radius ratio defaults to `10x`
...
- DRC Rule Check – Apply optimistic routing first, then progressively reduce or revert non-compliant corners. The maximum adjustment count defaults to `60` and is configurable from `1` to `100`; copper-pour rules can be ignored, and one final check always runs after all beautify post-processing
